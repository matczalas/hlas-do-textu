...
class _Segmented(QWidget):
    """Pill-style segmentovaný přepínač.

    Aktivní tlačítko má accent background + bílý text přes QSS
    [checked="true"] selector. Žádný overlay indikátor — v1.4.1
    odebrán protože jeho geometry animace kolidovala s parent
    layoutem a click events občas spadly mimo cíl.
...
class _ActionCard(QFrame):
    ...
    def __init__(
        ...
        *,
        ...
        # Hover lift přes QPropertyAnimation odebrán (v1.4.1) — způsoboval
        # seking layoutu při hover (QFrame v QVBoxLayoutu s animovanou
        # geometry kolidovala s drop shadow effect a click events občas
        # nepropadly na CTA tlačítko). Hover effect je teď čistě v QSS
        # přes [hover] pseudo-state na ActionCard v app.qss.
